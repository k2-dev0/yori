...
- 文書計画の適用はactive generationのspec検証より先に行う。pending revisionがある場合だけ世代を作成/検証し、spec不一致・retired/failedは自動切替せず`embedding_generation_mismatch`の恒久失敗にする。pendingが無ければ世代の作成/検証は不要としてlease条件付きcompletedにする。世代不一致でも制限的変更のpublication削除・is_searchable更新・revision状態は外部HTTP前に反映し、除外対象を古い公開として残さない。
