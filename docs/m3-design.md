...
- relation_explicitは候補ごとの独立したChoice質問とし、instructionsに対象候補と判断基準を明記する。集約時はrelation_targetで選択した候補に対応する高信頼回答だけを採用し、回答が低信頼または欠落なら関係を確定しない。質問版は`m3-2`とし、複数候補・単一候補・低信頼・入力予算・cache境界の回帰testで検証する。
