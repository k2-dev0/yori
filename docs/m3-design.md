...
- 外部評価後に比較対象の受付を同じIDで読み直し、評価した入力revisionの有効性を確認する。chain終端はnew_search確定済みに限定し、検索要否未判定のpending受付は再利用しない。
...
- 再利用の再修正後、Jev評価中の改訂と未判定受付の再利用は回帰testで解消を確認。ただし再取得・検証後からapplyRouteDecisionの保存までに先行入力が改訂される競合は未解決。保存TX内で先行入力revisionの有効性を保証する対応が必要で、再利用の競合対策全体は完了扱いにしない。
