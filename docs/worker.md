...
- payloadの`search_request_id`、job対象message/revision、job session、会社・案件・社員・session、`new_search`をDB正本で照合し、対象受付だけを`running`へする。障害更新・retryでも同じscope照合を行い、不一致payloadやmessage所属と異なるjob sessionでは外部送信・受付更新を行わない。詳細は[m5-design.md](m5-design.md)。
